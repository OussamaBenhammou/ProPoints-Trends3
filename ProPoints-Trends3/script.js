...
function addTask() {
    ...
    doneButton.onclick = function () {
        ...
        // Check if the task is not empty before proceeding
        if (taskText.trim() !== '') {
            // Display the task text instead of the textarea
            const taskTextDisplay = document.createElement('div');
            taskTextDisplay.textContent = taskText;

            taskContent.removeChild(taskTextarea);
            taskContent.appendChild(taskTextDisplay);

            // Remove the "Done" button
            taskContent.removeChild(doneButton);

            // Save tasks and remove the taskElement
            saveTasks();
        } else {
            alert('Veuillez saisir un objectif valide avant de cliquer sur "Done".');
        }
    };
...
function saveTasks() {
    ...
    taskContainer.querySelectorAll('.task .task-content').forEach(function (taskContent) {
        const taskText = taskContent.querySelector('div') ? taskContent.querySelector('div').textContent : '';
        tasks.push(taskText);
    });
